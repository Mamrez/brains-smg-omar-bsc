...
class TransformerModelDropNorm(nn.Module):
    def __init__(self, model_structure: dict):
        super(TransformerModelDropNorm, self).__init__()
    ...
    def build_model_structure(self, model_structure: dict):
        ...
        self.feature_embedding = nn.Linear(self.input_features, self.hidden_size)
        self.transformer = nn.Transformer(d_model=self.hidden_size, nhead=self.num_heads, num_encoder_layers=self.num_layers, dropout=self.dropout)
    ...
    def structure_consistency_check(self, model_structure: dict):
        ...
        if "dropout" not in model_structure:
            model_structure["dropout"] = default_dropout
            warnings.warn(
                "The model loaded does not define the dropout as expected. Changed it to default value: {}.".format(default_dropout)
            )
        else:
            dropout = model_structure.get('dropout')
            assert isinstance(dropout, float) and 0 <= dropout < 1, "dropout must be a float between 0 and 1"
