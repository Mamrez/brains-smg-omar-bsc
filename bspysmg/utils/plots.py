...
def plot_wave_prediction(io_file_path: str, predictions: np.array, data_type: str, save_directory: str,start_index: int, all_targets: np.array) -> None:
    """
    Plots I/O data and predictions for the model. Image is saved for each batch and it is used to control 
    what is happening to the device input-output relationship for each batch.

    Parameters
    ----------
    io_file_path : str
        Path to the IO.dat file containing the input and output data.
    predictions : np.array
        Predicted output generated by the model corresponding to the inputs.
    batch : int
        The current batch number of the data.
    save_directory : str
    ...
    for i in range(inputs.shape[1]):
        plt.plot(inputs[start_index:end_index, i], label=f'Input {i}')
    plt.ylabel('Inputs (V)')
    plt.xlabel('Time points (a.u.)')
    plt.legend(loc='upper right')
    
    # Plot outputs and predictions
    plt.subplot(212)
    plt.plot(all_targets, label='Actual Output')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_directory, f'example_' + data_type + '_prediction.png'))
    plt.close()
