...
def get_adjustable_frequency(configs: dict) -> np.array:
    """
    Generate adjustable input frequencies for each electrode.
    ...
    configs: dict
        ...
        1. input_frequency: list
           ...
           length as the activation electrode number. The input frequency list will be
           square rooted.
        2. base_input_frequency_multiplier: float
           A multiplier for adjusting the frequency to optimal device operation.

    Returns
    ---------
    np.array
        List of input frequencies for each electrode.
    """

    aux = np.array(configs['input_frequency'])[:, np.newaxis]

    sqrt_frequencies = np.sqrt(aux[:configs['activation_electrode_no']])

    max_freq = np.max(sqrt_frequencies) * configs['base_input_frequency_multiplier']

    # Check against the Nyquist frequency, which is half the sampling rate
    if max_freq >= configs['sampling_frequency'] / 2:
        # Calculate the maximum frequency that fulfills the Nyquist criterion
        adjusted_max_frequency = (configs['sampling_frequency'] / 2) / np.max(sqrt_frequencies)
        print(f"Given Frequency too high! Adjusted to {adjusted_max_frequency}.")
        # Use the adjusted frequency multiplier instead
        return sqrt_frequencies * adjusted_max_frequency
    else:
        # Use the original multiplier if the Nyquist criterion is satisfied
        return sqrt_frequencies * configs['base_input_frequency_multiplier']
