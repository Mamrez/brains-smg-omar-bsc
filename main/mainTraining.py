...
inputs, outputs, info_dictionary = post_process('brains/main/mainSamplingData/', clipping_value=None)
print(f"max out {outputs.max()} max min {outputs.min()} shape {outputs.shape}")
      

smg_configs = load_configs('brains/configs/training/smg_configs_template_omar.yaml')
